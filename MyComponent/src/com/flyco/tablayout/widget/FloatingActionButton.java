...
import com.pachong.mycomponent.R;
...
public class FloatingActionButton extends ImageButton {
	...
	void init(Context context, AttributeSet attributeSet) {
		...
		mSize = attr.getInt(R.styleable.FloatingActionButton_fab_size,
				SIZE_NORMAL);
		mIcon = attr
				.getResourceId(R.styleable.FloatingActionButton_fab_icon, 0);
		mTitle = attr.getString(R.styleable.FloatingActionButton_fab_title);
		mStrokeVisible = attr.getBoolean(
				R.styleable.FloatingActionButton_fab_stroke_visible, true);
		attr.recycle();
	...
	void updateBackground() {
		...
		layerDrawable.setLayerInset(1, circleInsetHorizontal, circleInsetTop,
		...
		layerDrawable.setLayerInset(3, circleInsetHorizontal + iconOffset,
				...
				circleInsetHorizontal + iconOffset, circleInsetBottom
						+ iconOffset);
	...
	private StateListDrawable createFillDrawable(float strokeWidth) {
		...
		drawable.addState(new int[] {},
				createCircleDrawable(mColorNormal, strokeWidth));
		return drawable;
	}
	...
	private Drawable createCircleDrawable(int color, float strokeWidth) {
		...
		Drawable[] layers = { fillDrawable,
				createInnerStrokesDrawable(opaqueColor, strokeWidth) };

		LayerDrawable drawable = alpha == 255 || !mStrokeVisible ? new LayerDrawable(
				layers) : new TranslucentLayerDrawable(alpha, layers);
	...
	private static class TranslucentLayerDrawable extends LayerDrawable {
		...
		public void draw(Canvas canvas) {
			...
			canvas.saveLayerAlpha(bounds.left, bounds.top, bounds.right,
					bounds.bottom, mAlpha, Canvas.ALL_SAVE_FLAG);
			super.draw(canvas);
			canvas.restore();
		}
	}
	...
	private int halfTransparent(int argb) {
		return Color.argb(Color.alpha(argb) / 2, Color.red(argb),
				Color.green(argb), Color.blue(argb));
	}
			...
			public Shader resize(int width, int height) {
				return new LinearGradient(width / 2, 0, width / 2, height,
						new int[] { topStrokeColor,
								topStrokeColorHalfTransparent, color,
								bottomStrokeColorHalfTransparent,
								bottomStrokeColor }, new float[] { 0f, 0.2f,
								0.5f, 0.8f, 1f }, TileMode.CLAMP);
			}
		});
