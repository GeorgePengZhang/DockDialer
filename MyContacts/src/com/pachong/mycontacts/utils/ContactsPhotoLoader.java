...
public class ContactsPhotoLoader {
	...
	// final int contactid, long photoid, int defaultRes) {
	...
	public void loadImage(final Context context, final ImageView imageView,
			final Uri uri, long photoid, int defaultRes) {
		imageView.setImageResource(defaultRes);
		if (photoid > 0 && uri != null) {
			...
			getExecutorService().execute(new Runnable() {
				...
				public void run() {
					Bitmap bmp = null;
					try {
						bmp = ContactsUtils.loadContactPhotoThumbnail(context, uri);
					} catch (Exception e) {
					}
					if (bmp == null) {
						try {
							bmp = cacheBitmap(context, uri);
						} catch (Exception e) {
						}
					}
					if (bmp == null) {
	...
	private Bitmap cacheBitmap(Context context, Uri uri) {
		Bitmap bmp = null;
		...
		try {
			...
			if (is != null) {
                ...
                int smallerExtentFromBytes = BitmapUtil.getSmallerExtentFromBytes(bytes);
                int sampleSize = BitmapUtil.findOptimalSampleSize(smallerExtentFromBytes, 60);
                bmp = BitmapUtil.decodeBitmapFromBytes(bytes, sampleSize);
            } 
		} catch (Exception e) {
			e.printStackTrace();
		} 
